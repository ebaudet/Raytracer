...
int		color_find(int color, t_data *d, t_vector *ray_dir, double coef)
{
	double		lambert;
	t_vector	impact;
	t_vector	normal;
	...
	normal.x = impact.x - d->sphere->position->x;
	normal.y = impact.y - d->sphere->position->y;
	normal.z = impact.z - d->sphere->position->z;
